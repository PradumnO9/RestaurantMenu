...
def __notify_public(self:WebSocket, input_data: int):
    # switch to discuss what to do when 
    match input_data:
        case 1 :
            self.send("case when 1")
        case 2 :
            self.send("case when 2")
        case 3 :
            self.send("case when 3")
    return
...
def get_read_only_session():
    try:
        ...
        return JSONResponse(content = {"message":"connection to database not made "} , status_code = 200, headers = None, media_type = None, background= None)

def get_read_write_session():
    try:
...
def eta():
    ...
    yield datetime.now().__add__(timedelta(0,0,0,0,0,1,0)) 
...
def __post_login_details(data : dict ):
    ...
    hash_password:str=data["hash_password"] 
    with open('./app/admin.ini','r') as admin_file:
        ...
        if  authenticate:
            ...
            return JSONResponse( content = {"type":"admin","bearer_token": base_token, "isLoggedIn":authenticate} , status_code = 200, headers = None, media_type = None, background= None)
...
def __post_add_menu_foodregionaltype(data:dict ):
    """
    ...
    try:
        result = None
        with Session(READONLYENGINE) as readonlysess:
            q_fetch_menucategory = select(FoodRegionalType.FoodRegionalTypeId).where(FoodRegionalType.FoodRegionalTypeName == data["FoodRegionalTypeName"])
        ...
        if not result:  
            with Session(READWRITEENGINE) as readwritesess:
                ...
                thiscategory.FoodRegionalTypeName=data["FoodRegionalTypeName"]
...
@mainapp.delete("/api/admin/menucategory/{FoodCategoryId}")
def __delete_menu_food(FoodCategoryId:int):
    with Session(READWRITEENGINE) as readwritesess:
        try:
            q_fetch_menucategory = select(FoodRegionalType.FoodRegionalTypeId, FoodRegionalType.FoodRegionalTypeName)
            result = readwritesess.exec(q_fetch_menucategory)
            thiscategory= q_fetch_menucategory.one()
            readwritesess.delete(thiscategory)
            readwritesess.commit()
            JSONResponse(content =  {"message":"delete successful"}, status_code = 200, headers = None, media_type = None, background= None)
        except:
            return JSONResponse(content =  {"message": "there was an issue deleting data", "type":"admin"}, status_code = 200, headers = None, media_type = None, background= None)
    ...
    try:
        with Session(READWRITEENGINE) as readwritesess:
            try:
                ...
                result = readwritesess.exec(q_fetch_menu_with_id)
                thisfood = result.one()
                if getattr(thisfood, Data["column"]) != None:
                    setattr(thisfood, Data["column"], Data["value"])
                    thisfood.commit()

                return JSONResponse(  content =  'updated successful', status_code = 200, headers = None, media_type = None, background= None)
            except FastAPIError as  e:
                return Response(content = {"message":"An error occurred while querying data."}, status_code=500,headers = None, media_type = None, background = None)
...
def __put_table_orders(table_id:int,table_qr_session_id:str , Data: dict):
    """
    ...
    try:
        with Session(READWRITEENGINE) as readwritesess:
            try:
                q_fetch_tableorder_with_id = select(Orders.OrderId,Customer.CusName,Orders.Dinetable_FId,Orders.Transaction_FId,Orders.OrderTimeStamp,Dinespace.DinespaceName).where(((Orders.Customer_FId==Customer.CusId) and Orders.Dinetable_FId == DineTable.DineTableId) and DineTable.Dinespace_FId == Dinespace.DinespaceId)
                result = readwritesess.exec(q_fetch_tableorder_with_id)
                object = result.one()
                if getattr(object,Data["name"]) != None:
                    setattr(object,Data["name"], Data["value"])
                readwritesess.commit(object)
                readwritesess.refresh(object)
                return JSONResponse(content=jsonable_encoder(object.mappings().all()),status_code=200,headers=None,media_type=None,background=None)
            except FastAPIError as e :
                return JSONResponse(content={"message":"An error occurred while querying data.", "type":"admin"}, status_code=500)
    except FastAPIError as t:
        return JSONResponse(content={"message":"Database Connectivity Issue"}, status_code=404)
...
# #         try:
# #             with Session(READWRITEENGINE) as readwritesess:
# #                 q_fetch_table_realtime = select()
# #                 readwritesess.exec()
# #         except FastAPIError as e:       
# #             return Response(content = {"message":"An error occurred while querying data.", "type":"admin"}, status_code = 200, headers = None, media_type = None, background= None)
...
def __delete_menu_food(FoodID:int):
    try:
        with Session(READWRITEENGINE) as readwritesess:
            try:
                q_delete = select(Food).where(Food.FoodId ==FoodID )
                result = readwritesess.exec(q_delete)
                thisfood = result.one()
                readwritesess.delete(thisfood)
                readwritesess.commit()
                return JSONResponse(content =  'Deleted Successfully', status_code = 200, headers = None, media_type = None, background= None)
            except FastAPIError as  e:
                return Response(content = "An error occurred while deleting data.", status_code=404)
            readwritesess.close()
    except FastAPIError  as t:
...
# @mainapp.get("/api/menuitem/{FoodID}")
# def __get_menu_food(FoodID:int):
#     try:
#         with Session(READONLYENGINE) as readonlysess:
#             try:
